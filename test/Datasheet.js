    ...
    describe('rendering', () => {
      it('should properly render', () => {
        ...
        expect(wrapper.html()).toEqual(
          shallow(
            <td className="test cell" colSpan={5} rowSpan={4}>
  ...
  describe('ComponentCell component', () => {
    describe('rendering', () => {
      it('should properly render', () => {
        ...
        expect(wrapper.html()).toEqual(
          shallow(
            <td className="test cell updated" colSpan={5} rowSpan={4}>
              7
            </td>
          ).html()
        );
    ...
    describe('rendering', () => {
      it('should properly render a change (flashing)', done => {
        ...
        expect(wrapper.html()).toEqual(
          shallow(
            <td className="test cell updated" colSpan={1} rowSpan={1}>
              <div>HELLO</div>
            </td>
          ).html()
        );
  ...
  describe('DataSheet component', () => {
    ...
    describe('rendering with varying props', () => {
      ...
      it('renders proper elements by column', () => {
        ...
        expect(
          customWrapper.find('td > span').nodes.map(n => n.innerHTML)
        ).toEqual([
      ...
      it('renders a cell with readOnly field properly', () => {
        customWrapper = mount(
          <DataSheet
            data={[
              [{ data: 12, readOnly: true }, { data: 24, readOnly: false }]
            ]}
            valueRenderer={cell => cell.data}
            dataRenderer={cell => '=+' + cell.data}
            onChange={(cell, i, j, value) => (data[i][j].data = value)}
        ...
        expect(customWrapper.find('td.cell input').at(0).html()).toEqual(
          '<input style="display: none;">'
        );
      });
    ...
    describe('selection', () => {
      ...
      it('calls onSelect prop when a new element is selected', done => {
        customWrapper = mount(
          <DataSheet
            data={data}
            onSelect /* eslint-disable keyword-spacing */={cell => {
              try {
    ...
    describe('editing', () => {
      ...
      it('does not start editing if cell is readOnly', () => {
        wrapper.setProps({
          data: [[{ data: 1, readOnly: true }, { data: 2, readOnly: true }]]
        });
        //[0  , 9 ,a , z , 0 , 9  , +  , = , decim]
        [48, 57, 65, 90, 96, 105, 107, 187, 189].map(charCode => {
          cells.at(0).simulate('mousedown');
      ...
      it('doesnt auto paste data if cell is editing', () => {
        const datacust = [
          [{ data: 12, readOnly: false }, { data: 24, readOnly: false }]
        ];
        customWrapper = mount(
          <DataSheet
            data={datacust}
      ...
      it('pastes data properly and fires onPaste function if defined', done => {
        ...
        customWrapper = mount(
          <DataSheet
            ...
            valueRenderer={cell => cell.data}
            onChange={(cell, i, j, value) => (datacust[i][j].data = value)}
            onPaste /* eslint-disable keyword-spacing */={pasted => {
              try {
                expect(pasted).toEqual([
                  [
